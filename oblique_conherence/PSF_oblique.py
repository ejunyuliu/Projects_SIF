...
if __name__ == '__main__':
    ...
    plt.figure()
    for oblique in range(0, 90, 10):
        zpp = 10
        iPSF_integ = 0
        for phi in range(0, 360, 10):
            phi_ma, phi_de, phi_diff, iPSF_cos, iPSF = iPSF_misalignment(xv, yv, k, 0, 0, zpp, 0.3, np.pi / 2,
                                                                         oblique, phi)
            iPSF_integ = iPSF_integ + iPSF

        plt.plot(iPSF_integ[100, 80:120], label='oblique=' + str(oblique))
